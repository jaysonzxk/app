  ...
  <view class="vip" :style="{'padding-top': statusBarHeight + 'px'}">
    ...
    <view class="body">
      ...
      <view class="vip-list">
        ...
        <view class="vip-card">
          <view class="vip-item" v-for="(v,index) in vipList" :key="v.id" @click="changeIndex(v,index)"
                >
            <view class="item" :class="{'active' : isActive ? indexI === index : !isActive}">
              <view class="info">
      ...
      <view class="pay-list">
        ...
        <view class="pay-item" v-for="(p,index) in payChannelList" :key="p.id" @click="changePay(p.id,index)">
          <view class="item" :class="{'pay-active': payActive ? payIndex === index : !payActive}">
            <span>{{p.name}}</span>
            <span class="icon"></span>
          </view>
        </view>
...
import {getVipList, getPayChannel} from "@/common/api"
    ...
    .vip-list {
      ...
      .vip-card {
        ...
        .vip-item {
          ...
          .active {
            border: 1px solid rgb(212, 119, 60) !important;
            //.item{
